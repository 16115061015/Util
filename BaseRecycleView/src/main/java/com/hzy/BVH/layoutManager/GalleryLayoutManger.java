...
public class GalleryLayoutManger extends RecyclerView.LayoutManager {
     ...
     * 缩放比例
     */
    private float mIntervalScale = 1;
    ...
    public GalleryLayoutManger(boolean isFlat, boolean isGreyItem,
                               boolean isAlphaItem, float cstInterval, float csScale) {
        mIsFlatFlow = isFlat;
        ...
        if (csScale >= 0 && csScale <= 1) {
            mIntervalScale = csScale;
        }
    }
    ...
    private void layoutItems(RecyclerView.Recycler recycler,
        ...
        int position = 0;
        Log.i("forTest", "现在有的子View:"+getChildCount());
...
//            cm.setSaturation(0.9f);
    ...
    private float computeScale(int x) {
        float scale = 1 - (Math.abs(x - mStartX) * 1.0f / Math.abs(mStartX + mDecoratedChildWidth / mIntervalRatio) * mIntervalScale);
        if (scale < 0) scale = 0;
        if (scale > 1) scale = 1;
        return scale;
    }
    ...
    public static class Builder {
        ...
        public Builder setIntervalScale(float scale) {
            cstIntervalScale = scale;
            return this;
        }

        public GalleryLayoutManger build() {
            return new GalleryLayoutManger(isFlat, isGreyItem,
                    isAlphaItem, cstIntervalRatio, cstIntervalScale);
        }
    }
}